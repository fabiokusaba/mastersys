...
import dev.fabiokusaba.mastersys.domain.FaturaMatricula;
import dev.fabiokusaba.mastersys.projection.AlunosPorCidadeProjection;
import dev.fabiokusaba.mastersys.projection.FaturamentoMensalProjection;
import dev.fabiokusaba.mastersys.projection.FaturasEmAbertoProjection;
...
public interface RelatorioAcademiaRepository extends Repository<FaturaMatricula, Long> {
    @Query(
            value = """
            SELECT TO_CHAR(data_vencimento, 'YYYY-DD') AS mes, SUM(valor) AS total
    ...
    @Query(
            value = """
            SELECT m.id AS matriculaId, a.nome as alunoNome, f.valor
