...
FROM matriculas m
JOIN modalidades ON mo.nome = 'Musculação'
...
FROM matriculas m
JOIN modalidades ON mo.nome = 'Jiu-Jitsu'
...
SELECT m.id, CURRENT_DATA - INTERVAL '60 days', 120.00,
       CURRENT_TIMESTAMP - INTERVAL '58 days', 'PAGA'
FROM matriculas m
WHERE m.aluno_id = 1;

INSERT INTO faturas_matriculas(matricula_id, data_vencimento, valor, status)
SELECT m.id, CURRENT_DATA - INTERVAL '10 days', 120.00, 'ABERTA'
FROM matriculas m
WHERE m.aluno_id = 1;

INSERT INTO faturas_matriculas(matricula_id, data_vencimento, valor, data_pagamento, status)
SELECT m.id, CURRENT_DATA - INTERVAL '30 days', 180.00,
    CURRENT_TIMESTAMP - INTERVAL '28 days', 'PAGA'
FROM matriculas m
WHERE m.aluno_id = 2;

INSERT INTO faturas_matriculas(matricula_id, data_vencimento, valor, status)
SELECT m.id, CURRENT_DATA - INTERVAL '15 days', 180.00, 'ABERTA'
FROM matriculas m
WHERE m.aluno_id = 2;